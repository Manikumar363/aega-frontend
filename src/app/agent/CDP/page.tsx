...
export default function AgentCDPPage() {
  ...
  const fetchCourses = async () => {
    try {
      ...
      // Filter courses specific to Agents / B2B
      const agentCourses = data.filter(
        (c: any) => !c.courseFor || c.courseFor === "agents" || c.courseFor === "b2b"
      );

      const normalized = agentCourses.map((c) => {
  ...
  const getImageSrc = (path?: string) => {
    if (!path) return "https://images.unsplash.com/photo-1503676260728-1c00da094a0b?auto=format&fit=crop&w=400&q=80";
  ...
  // Open Registration / Launch Modal
  ...
  // Submit Registration / Launch Course
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedCourse) return;

    if (!startDate) {
      toast.error("Start Date is mandatory to register for a course.");
      return;
    }
  ...
  // Handle Certificate Upload with 10MB Limit Validation
    ...
    <DashboardLayout role="agent">
      <div className="space-y-8 text-white pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-800 pb-4">
          <div>
            <h1 className="text-3xl font-bold">CDP Training Platform</h1>
            <p className="text-sm text-white/60">
              Complete mandatory and optional Agent CPD training modules.
            </p>
          </div>
        ...
        {/* DYNAMIC CDP PROGRESS SCORE BAR SECTION */}
        <div className="bg-[#14112E] border border-gray-800 rounded-xl p-6 space-y-4 shadow-xl">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-400">Monthly CPD completion and certification score bar</p>
            </div>
        ...
        <div className="space-y-4">
          ...
          {loading ? (
            ...
            <div className="bg-[#14112E] border border-gray-800 p-8 rounded-xl text-center text-gray-400">
              No CDP courses available for Agent profile.
            </div>
          ) : (
        ...
        {/* 1. COURSE REGISTRATION MODAL (ORDER: Image -> Scrollable Description -> Start Date & Notes -> Actions) */}
        {isRegistrationModalOpen && selectedCourse && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
            <div className="bg-[#14112E] border border-gray-700 rounded-xl p-6 max-w-lg w-full max-h-[85vh] overflow-y-auto text-white space-y-5 shadow-2xl">
              ...
              {/* 2. Below Description with Scrollable Box */}
              <div>
              ...
              {/* 3. Mandatory Start Date & Optional Notes */}
          ...
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
            <div className="bg-[#14112E] border border-gray-700 rounded-xl p-6 max-w-md w-full text-white space-y-4 shadow-2xl">
              <div className="flex justify-between items-center border-b border-gray-800 pb-3">
                <h3 className="text-base font-bold text-[#F68E2D]">Edit Schedule for {selectedCourse.courseName}</h3>
              ...
              <form onSubmit={handleEditSubmit} className="space-y-4 text-xs">
                ...
                <div className="flex justify-end gap-3 pt-3 border-t border-gray-800">
                  ...
                  <button
                    type="submit"
                    disabled={submitting}
                    className="px-6 py-2 bg-[#F68E2D] hover:bg-[#e28124] rounded-lg text-xs font-bold uppercase disabled:opacity-50"
                  >
        ...
        {/* 3. WATCH VIDEO MODAL */}
        {isVideoModalOpen && selectedCourse && watchVideoUrl && (
          <div className="fixed inset-0 bg-black/85 flex items-center justify-center p-4 z-50">
            <div className="bg-[#14112E] border border-gray-700 rounded-xl p-6 max-w-3xl w-full text-white space-y-4 shadow-2xl">
              <div className="flex justify-between items-center border-b border-gray-800 pb-3">
                <h3 className="text-base font-bold text-white">{selectedCourse.courseName} - Video Module</h3>
                <button onClick={() => setIsVideoModalOpen(false)} className="text-gray-400 hover:text-white font-bold">
                  <X className="w-5 h-5" />
                </button>
              </div>

              <div className="h-96 w-full rounded-lg overflow-hidden bg-black flex items-center justify-center border border-gray-800">
                {watchVideoUrl.includes("youtube") || watchVideoUrl.includes("vimeo") ? (
                  <iframe src={watchVideoUrl} className="w-full h-full" allowFullScreen />
                ) : (
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="px-6 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-xs font-bold"
                >
                  Close Video
                </button>
              </div>
        ...
        {/* 4. COURSE COMPLETION CERTIFICATE MODAL */}
        {isCompletionModalOpen && selectedCourse && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
            <div className="bg-[#14112E] border border-gray-700 rounded-xl p-6 max-w-md w-full max-h-[85vh] overflow-y-auto text-white space-y-4 shadow-2xl">
              <div className="flex justify-between items-center border-b border-gray-800 pb-3">
                <h3 className="text-base font-bold text-emerald-400">Complete {selectedCourse.courseName}</h3>
                <button onClick={() => setIsCompletionModalOpen(false)} className="text-gray-400 hover:text-white">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleCompletionSubmit} className="space-y-4 text-xs">
                <div>
                  <label className="block text-gray-300 font-semibold mb-1">
                    Upload Certificate File (PDF / Image &lt; 10MB)
                  </label>
                  <input
                    type="file"
                    accept=".pdf,.jpg,.png"
                ...
                <div>
                  <label className="block text-gray-300 font-semibold mb-1">Completion Notes</label>
                  <textarea
                    rows={3}
                ...
                <div className="flex justify-end gap-3 pt-3 border-t border-gray-800">
                  ...
                  <button
                    type="submit"
                    disabled={completing}
                    className="px-6 py-2 bg-emerald-600 hover:bg-emerald-700 rounded-lg text-xs font-bold uppercase disabled:opacity-50 text-white"
                  >
