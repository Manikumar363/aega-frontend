			...
			{stats.map((row, rowIdx) => (
				<div key={rowIdx} className="grid grid-cols-1 md:grid-cols-3 border border-[#3A3760]">
					{row.map((card, colIdx) => (
						<div
							key={colIdx}
							className={`bg-[#14123A] px-6 py-5 flex flex-col gap-4 ${colIdx < row.length - 1 ? "border-r border-[#3A3760]" : ""
								}`}
						>
				...
				<div className="bg-[#0F0D2B] border border-[#3A3760]">
					{indicators.length === 0 ? (
						...
						indicators.map((item, idx) => (
							<div
								key={idx}
								className={`flex items-center justify-between px-5 py-4 ${idx < indicators.length - 1 ? "border-b border-[#2D2A50]" : ""
									}`}
							>
