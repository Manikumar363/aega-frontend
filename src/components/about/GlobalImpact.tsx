...
interface GlobalImpactProps {
  data?: {
    description?: string;
    kpis?: Array<{ value: string; description: string }>;
  };
}
...
export default function GlobalImpact({ data }: GlobalImpactProps) {
  const description = data?.description || "AEGA's influence spans 45+ countries, supporting thousands of agents, partnering with leading universities, and protecting hundreds of thousands of students worldwide.";
  
  const stats = Array.isArray(data?.kpis) && data.kpis.length > 0 ? data.kpis : [
  ...
  return (
    <section className="relative w-full bg-[#03091F] overflow-hidden pb-20">
      ...
      <div className="relative flex flex-col md:flex-row items-center justify-center gap-10 md:gap-16 mb-16 px-4">
        ...
        {stats.slice(0, 4).map((stat, i) => (
          <div key={i} className="flex flex-col items-center justify-center min-w-[120px]">
            ...
            <span className="text-white/80 text-base md:text-lg text-center font-medium capitalize">{stat.description}</span>
          </div>
        ))}
      </div>
      ...
      <div className="mb-16 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {REGIONS.map((region, i) => (
            <div key={region.name} className="border border-white/20 p-6 flex flex-col gap-2 bg-[#0A1628]/60 text-left">
      ...
      <div className="relative max-w-3xl mx-auto rounded-lg overflow-hidden shadow-2xl px-0">
        ...
        <div className="relative z-10 p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-6 bg-black/60 text-left">
          <div>
