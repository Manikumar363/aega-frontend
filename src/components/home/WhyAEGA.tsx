  ...
  return (
    <section className="relative py-20 bg-zinc-800 flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center max-w-4xl px-4">
        ...
        <p className="text-xl md:text-xl text-white/80 leading-relaxed font-light drop-shadow-md whitespace-pre-line">
          {description}
        </p>
          ...
          {bottomTitles.map((title, idx) => (
            title ? (
              <div key={idx} className="flex items-center gap-3">
                ...
                <span className="text-white/60 font-bold text-lg md:text-xs uppercase">{title}</span>
              </div>
            ) : null
          ))}
        </div>
