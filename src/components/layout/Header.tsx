...
const NavItem = ({
  ...
  // Determine if this NavItem is active
  const isActive = hasDropdown && dropdownItems
    ? dropdownItems.some(item => pathname === item.href || (item.href !== '/' && pathname.startsWith(item.href)))
    : href === '/'
      ? pathname === '/'
    ...
    >
      ...
      {!hasDropdown && href ? (
        <Link href={href} className="flex items-center gap-1 h-full">
          <span className={`text-[9px] font-bold tracking-widest transition-colors ${isActive ? 'text-[#F58A07]' : 'text-gray-200 group-hover:text-[#F58A07]'}`}>
            {label}
          </span>
        </Link>
      ) : (
        <div className="flex items-center gap-1 h-full cursor-pointer">
          <span className={`text-[9px] font-bold tracking-widest transition-colors flex items-center gap-1 ${isActive ? 'text-[#F58A07]' : 'text-gray-200 group-hover:text-[#F58A07]'}`}>
            {label}
      ...
      {/* Hover Underline */}
      <span className={`absolute bottom-[30%] left-3 right-3 h-0.5 bg-[#F58A07] transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} />
        ...
        <div className="absolute top-full left-0 pt-4 z-50">
          <div className="bg-[#0F1A3A] border border-white/10 shadow-2xl min-w-[220px] py-2">
            {dropdownItems.map((item, index) => {
              const isChildActive = pathname === item.href || (item.href !== '/' && pathname.startsWith(item.href));
              return (
                <Link
                  key={index}
                  href={item.href}
                  className={`block px-6 py-3 text-xs font-medium transition-colors ${isChildActive ? 'bg-[#1A2547] text-[#F58A07]' : 'text-gray-300 hover:bg-[#1A2547] hover:text-[#F58A07]'}`}
                >
                  {item.label}
                </Link>
              );
